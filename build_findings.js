    ...
    new Table({
      ...
      rows: [
        ...
        ...([
          ...
          ["RM feature adoption walkthrough", "In progress — paused at Inspections"],
          ["Maintenance closeout & turnover review", "In progress, Findings 09-10 logged"],
          ["St. Paul PM discovery call", "Scheduled — Fri Jun 20"],
          ["Minneapolis PM discovery call (Jocelyn)", "Complete — Findings logged"],
          ["Synthesis & roadmap", "Pending findings completion"]
        ].map(([lane, status], i) =>
          new TableRow({ children: [
            new TableCell({ borders, width: { size: 4680, type: WidthType.DXA },
...
function buildFindings() {
  return [
    ...
    // ── FINDING 04 ────────────────────────────────────────────────────────────
    findingHeader("04", "Urgent Maintenance Requests Bypass Rent Manager Entirely", "High Impact"),
    spacer(40),
    findingTable([
      { label: "Source", value: "Minneapolis PM (Jocelyn) — discovery call, Jun 17" },
      { label: "The pain", value: "When a maintenance issue is urgent, residents text or call Jocelyn directly. She passes it to the maintenance team by text. The job gets handled, but by the time anyone thinks to log it in Rent Manager, the moment has passed — minor jobs often never get logged at all. Jocelyn also has limited visibility into how the maintenance team closes out work orders on their end, so she cannot tell from RM whether a job has actually been completed and documented." },
      { label: "The gap", value: "Urgent work — which by definition involves the most time-sensitive and potentially liability-relevant repairs — is the least likely to leave a paper trail. If a resident later disputes a repair, or the same issue recurs, there may be no record that anything was done. This is the same documentation risk Brian flagged in the initial call." },
      { type: "bullets", label: "Recommendation", values: [
        "Establish a simple rule: even urgent jobs get a work order opened in RM, even if it is created after the fact the same day. A one-line note and a status of Completed is enough to close the gap.",
        "Investigate whether RM's mobile experience (or a lightweight mobile entry point) makes it easier for the maintenance team to log jobs from the field, reducing the friction that causes logging to be skipped.",
        "Review the maintenance team's current RM habits directly — Jocelyn does not know how their closures appear in the system, which itself indicates a visibility gap worth closing."
      ]},
      { label: "Effort to fix", value: "Low on process (a team agreement and short training), medium if mobile access needs to be configured or improved." }
    ]),
    spacer(200),
    ...
    findingTable([
      ...
      { type: "bullets", label: "Recommendation", values: [
        "Begin using RM's Prospects module to log every inbound inquiry at first contact, with the lead source recorded (Zillow, website, referral, etc.).",
        "Use the Prospect Leasing Board to track stage progression — inquiry, contacted, showing scheduled, showing completed, applied.",
        "This single workflow change produces the leasing funnel Brian asked for, using a module already included in their subscription."
      ]},
      { label: "Effort to fix", value: "Low technically — the module is already there. Medium in practice — requires a habit change for whoever handles inbound leads, and a short training session on the Prospects workflow." }
    ]),
    spacer(200),
    ...
    findingTable([
      ...
      { label: "What the live review found", value: "The module is owned and the underlying Service Manager taxonomy is populated and actively used: eight make-ready actions and a 21-item issue-category list both exist in Service Setup. But the board has never been activated. On the audit login the template builder's Action, Category, Priority, and Status pickers return no results despite those lists being populated; the Service Setup lists are view-only (no edit control); and the one existing template is locked even with administrator rights. These point to a permissions or account-scoping issue, not absent configuration." },
      { label: "The gap", value: "Turnover lives in Excel not by preference but because the in-system tool the team already owns has never been usable to them: its access and scoping were never resolved. The capability is present; the configuration access is not." },
      { type: "bullets", label: "Why it matters", values: [
      ...
      { type: "bullets", label: "Recommendation", values: [
        "Resolve the access issue with Pergola's RM rep: why the template builder cannot see the populated lists, what role enables editing Service Setup, and how to unlock or relocate the existing template (and which account or location holds it).",
    ...
    // ── FINDING 10 ────────────────────────────────────────────────────────────
    findingHeader("10", "Logged Prospect Calls Do Not Appear in Communication History", "Quick Win"),
    spacer(40),
    findingTable([
      { label: "Source", value: "Live RM system review (Jun 18)" },
      { label: "Observed", value: "A prospect record shows a logged phone call, but the contact's Communication history is empty, and the \"Unlinked Calls\" view buffers indefinitely without loading. The call data appears to exist but is not linked to the record where it would be useful." },
      { label: "Why it matters", value: "Pergola pays for rmVoIP as part of the platform. If inbound and outbound calls are not reliably linking to prospect and tenant records, the call log (a natural part of the leasing funnel and communication history, see Finding 05) is incomplete, and staff cannot see a contact's full interaction history in one place." },
      { label: "Recommendation", value: "Raise with Pergola's RM rep as a likely rmVoIP sync or call-linking issue. Confirm whether calls are expected to auto-link to records, why the Unlinked Calls view fails to load, and whether a setting or re-sync resolves it." },
      { label: "Status", value: "Under investigation. Flagged to the RM rep; cause not yet confirmed." }
    ]),
    spacer(200),

    // ── Placeholder for future findings ───────────────────────────────────────
    new Paragraph({
      children: [new TextRun({ text: "Additional findings pending: St. Paul PM call (Jun 20), the maintenance-team interviews (Chris completed Jun 19, others scheduled), and the full feature adoption walkthrough (in progress). Findings 09 and 10 are logged and will be updated as the RM rep responds.", font: "Arial", size: 22, color: "999999", italics: true })],
...
function buildRoadmap() {
  return [
    sectionHeading("03", "Prioritized Roadmap"),
    rule(),
    new Paragraph({
      children: [new TextRun({ text: "This section will be completed upon conclusion of all discovery calls and system review. It will present findings ranked by impact and effort, with a phased implementation sequence distinguishing what the team can implement directly vs. what requires outside assistance.", font: "Arial", size: 22, color: "555555", italics: true })],
      spacing: { before: 60, after: 60 }
    }),
    spacer(120)
  ];
